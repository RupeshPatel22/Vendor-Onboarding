...
@Component({
  ...
  templateUrl: './grocery-history.component.html',
  styleUrls: ['./grocery-history.component.scss']
})
export class GroceryHistoryComponent implements OnInit {
  currentPage: number = 1;
  pageSize: number = 10;
  search: string;
  historyGroceryList: OutletCard[] = [];
  totalRecords: number;
  ...
  */
  viewOutlet(outletId) {
    this.router.navigate(['vendors/view-vendor', outletId], { queryParams: { service: Services.Grocery } });
  }
  /**
 ...
 */
  showHistoryOutletList(searchFlag?: boolean) {
    ...
    this.vendorService.getOutletsListForAdmin(data, Services.Grocery).subscribe(res => {
      this.historyGroceryList = [];
      for (const i of res['result']['stores']) {
        this.historyGroceryList.push(OutletCard.fromJson(i));
      }
